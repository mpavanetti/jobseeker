...
class jobSeeker:
    ...
    def __init__(self, environment="LOCAL",job=None):
        ...
        self.job = os.environ.get("JOB_NAME") or job
        self.transaction_open = False
        print("Job Name: ", self.job)
    ...
    def cancelOnSignal(self, signum, frame):
        self.cancel("Process interrupted by signal %s" % signum)
        sys.exit(128 + signum)
    ...
    def cancel(self,msg="Process cancelled"):
        if not getattr(self, "transaction_open", False):
            return False

        try:
            mycursor = self.mydb.cursor()
            sql = "UPDATE tmf SET status = 'cancelled', event_text = 'Cancelled', last_activity = now(), running_time = TIMEDIFF(now(), start_time), msg = %s WHERE instance_id = %s AND environment = %s AND job_name = %s AND LOWER(status) = 'running'"
            values = msg,self.id,self.environment,self.job
            mycursor.execute(sql,values)

            if(mycursor.rowcount == 1):
                print("[Transaction Cancelled] ",mycursor.rowcount, "record updated.")
                self.mydb.commit()
                self.transaction_open = False
                return True
            else:
                print("Some error has occured, no record has been cancelled, attempting to rollback operation.")
                self.mydb.rollback()
                return False
            mycursor.close

        except Exception as e:
            print(f"Error connecting to MariaDB Platform, error to cancel tmf transaction: {e}")
