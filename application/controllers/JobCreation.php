     ...
     if($jenkins_home === '' || $jenkins_home === null){
      ...
      private function ensureInlinePythonProjectFiles($pythonExecution, $runtimeOptions) {
        ...
        $bootstrapScript = implode("\n", array(
          ...
          '    echo "No Python interpreter is installed in the OpenVSCode container." >&2',
          '    exit 127',
          '  fi',
          '  JOBSEEKER_WORKSPACE_PYTHON=python3',
          '  JOBSEEKER_TARGET_PYTHON="$(python3 -c "import sys; print(str(sys.version_info[0]) + chr(46) + str(sys.version_info[1]))")"',
          'fi',
          ...
          'fi',
          'if [ "$JOBSEEKER_VENV_PYTHON_VERSION" != "$JOBSEEKER_TARGET_PYTHON" ]; then',
          '  rm -rf .venv',
          'fi',
          'OSTYPE="${OSTYPE:-}"',
          'export OSTYPE',
          ...
          '  . .venv/bin/activate',
          'fi',
          'python -c "import sys; expected = tuple(map(int, \"'.$workspacePythonVersion.'\".split(\".\"))); actual = sys.version_info[:2]; raise SystemExit(0 if actual == expected else \"Expected Python '.$workspacePythonVersion.' but selected {}.{}\".format(*actual))"',
