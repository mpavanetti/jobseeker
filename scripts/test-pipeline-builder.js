...
assert(controller.includes('validatePipelineCron'));
assert(controller.includes('cronSchedule->validateSpec'), 'Pipeline schedules must be validated with the shared JenkinsCronSchedule grammar.');
assert(controller.includes('public function validateSchedule()'), 'Pipelines must expose a live schedule validation endpoint.');
...
assert(controller.includes('prepareDeploymentJobs'));
assert(controller.includes('transformDeploymentJobXml'));
assert(controller.includes('rewriteDeploymentAgent'));
assert(controller.includes('copyDeploymentArtifacts'));
assert(controller.includes('copyDeploymentEnvironmentFiles'));
assert(controller.includes("'Repository payload for '.$targetJob.' already exists. Enable overwrite to replace it.'"));
assert(controller.includes('jenkinsCreateItemPath'));
assert(controller.includes('deployPreparedJobs'));
assert(controller.includes("'deployedJobs' => $jobDeployment['deployed']"));
assert(controller.includes('isTransientDeploymentPath'));
assert(controller.includes('RecursiveCallbackFilterIterator'));
// Legacy inline Python jobs baked the environment as a positional entrypoint arg.
assert(controller.includes('\\x27\\s+sh\\s+'), 'Deployment must rewrite the legacy baked Python environment argument.');
