...
assert(shellSamples >= 4, 'The catalog must keep at least four shell examples.');
assert(pythonSamples >= 6, 'The catalog must keep at least six Python/JobSeeker examples.');
...
assert(samples.includes("'files' => array(") && samples.includes('tests/test_rules.py'), 'The Python catalog must include a multi-file tested workspace.');
...
assert(samples.includes('jobseeker-connector exec warehouse') && samples.includes('jobseeker-asset orders-inbound'), 'Shell templates must cover the connector and Data Asset runtime helpers together.');
